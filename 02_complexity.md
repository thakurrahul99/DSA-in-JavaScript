...
1. **Big-O (O) — Upper Bound (Roof/Ceiling):** Yeh batata hai ki aapka code maximum kitne operations le sakta hai. Is bound ko rate of growth kabhi cross nahi karega. (It guarantees: *"Isse bura nahi hoga!"*)
2. **Big-Omega (Ω) — Lower Bound (Floor):** Yeh batata hai ki aapka code kam se kam kitne operations lega hi lega (minimum benchmark). (It guarantees: *"Isse behtar nahi chalega!"*)
3. **Big-Theta (Θ) — Tight Bound:** Jab upper bound aur lower bound bilkul same ho jayein, toh use tight bound kehte hain. Yeh exact rate of growth ko lock kar deta hai.
...
* **Best Case:** Target pehle hi element par mil jaye (`target = 10`). Operations = 1. Complexities: Upper Bound is O(1), Lower Bound is Ω(1), Tight Bound is Θ(1).
* **Worst Case:** Target aakhri index par ho ya array mein ho hi na (`target = 100`). Pure array ko traverse karna padega. Operations = n. Complexities: Upper Bound is O(n), Lower Bound is Ω(n), Tight Bound is Θ(n).
* **Average Case:** Target array ke beech mein kahin mile (around n/2 steps). Constant factor hatane par complexity is Θ(n).
...
```
* *Math Intuition:* Agar n = 16 hai, toh operations honge: 16 → 8 → 4 → 2 → 1 (sirf 4 steps, i.e., log_2 16 = 4).
...
```
* **Step-by-step Derivation:**
  * Outer loop i=0 par chala → Inner loop runs n times.
  * Outer loop i=1 par chala → Inner loop runs n times.
  * ...
...
```
* **Operation Count:** Loop variable i values lega: 0, 3, 6, 9, ... up to n.
* **Pattern:** Total loop steps = ≈ n/3.
* **Simplification:** 1/3 × n → Constants drop karne par, total complexity bachti hai: **O(n)** Time.
...
* **Dry Run values of `i`:**
  ...
  * Step 4: i = 8
  * Step k: i = 2^{k-1}
* **Derivation:** Loop tab tak chalega jab tak i < n hai. Yaani 2^{k-1} < n. Dono side standard base-2 log lagane par: k-1 ≈ log_2 n => k ≈ log_2 n.
...
* **Dry Run Trace Table:**
  ...
  * ...
  * i = n-1 → Inner loop runs n times.
* **Math Summation:** Total steps = 1 + 2 + 3 + ... + n.
  
  This is a standard Arithmetic Progression (AP) series:
  
  \\[Total Sum = n(n+1)/2 = n^2 + n/2\\]
  
* **Simplification:** Drop lower-order term (n) and ignore constant division factor (1/2):
...
1. **Rule of Dominance (Drop Lower-Order Terms):** Pure equation mein jo component sabse tez grow karega, sirf use hi select karenge.
   * O(n^2 + n) → n^2 is dominant → **O(n^2)**
   * O(2^n + n^5) → 2^n scales far faster → **O(2^n)**
   * O(n log n + n + 100) → n log n dominates → **O(n log n)**
...
* **Time Analysis:** Total call stack frames N linear chain mein execute hote hain, aur har frame ke andar sirf basic constant math calculations hote hain → **Time Complexity: O(n)**.
* **Space Analysis:** Jab tak base case execution resolve nahi hota, computer ko pure sequence call stacks chain variables ko memory par save rakhna padta hai → **Space Complexity: O(n) Auxiliary**.
...
1. **Step 1: Check the loop updater first:**
   * `i++` runs n times → O(n).
   * `i += 2` runs n/2 times → O(n).
   * `i *= 2` or `i /= 2` reduces the problem dynamically → O(log n).
2. **Step 2: Identify Loop Interdependency (Sequential vs. Nested):**
   ...
   * Nested loops with nested execution? Multiply bounds: O(n × m).
...
## 12. PRACTICE CORNER (EASY → MEDIUM → HARD)
...
1. **Operation Count:** Loop variable i increments step sequence bounds: 0, 2, 4, 6, ... up to n. Total operations in loop = n/2 steps.
2. **Derivation:** Mathematical equation for total steps: Steps = n/2 = 0.5n.
3. **Simplify:** Drop constant factor 0.5 → O(n) Time.
4. **Space Check:** Simple primitives used, no dynamic arrays allocations → O(1) space.
...
2. **Inner Loop Pattern:** Starts at j = n, updates via division: j = ⌊ j / 2 ⌋. This means the inner loop cuts problem size in half each step → Runs log_2 n times.
3. **Operations Summation:** Kyunki dono loops nested hain, hum nested multiplication rules follow karenge:
   
   \\[Total Operations = n × log_2 n = n log n\\]
...
3. **Total Operations:** At each level d, the number of branches doubles (2^0 → 2^1 → 2^2 ... up to level n). Total sum of geometric progression operations of tree vertices yields: O(2^n).
4. **Space (Stack Height):** At any given moment, the deepest active execution branch on the call stack is N. Call stack resolves sequentially from left branches first → Maximum simultaneous frames on call stack is N → O(n) Auxiliary Space.
...
2. **Galti 2: Confusing sequential loops addition with nested multiplication!**
   ...
   * *Reality:* Unki complexities **add** hongi: O(n) + O(n) = O(2n) → O(n), multiply hokar quadratic nahi ho jayegi.
...
| **O(2^n)** | Terrible / Exponential | 1.26 × 10^{29} computations | Naive Recursive Fibonacci algorithm. |
| **O(n!)** | Collapse / Factorial | 9.3 × 10^{157} computations | Set partitions, String permutations generation. |
...
⚠️ **Common Mistakes:** Forgetting call stack variables allocations during recursion depth tracing, and blindly naming independent variables as O(n^2) instead of O(n × m).
